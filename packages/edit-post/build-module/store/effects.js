...
var effects = {
  SET_META_BOXES_PER_LOCATIONS: function SET_META_BOXES_PER_LOCATIONS(action, store) {
    ...
    var wasAutosavingPost = select('core/editor').isAutosavingPost(); // Save metaboxes when performing a full save on the post.

    subscribe(function () {
      var isSavingPost = select('core/editor').isSavingPost();
      var isAutosavingPost = select('core/editor').isAutosavingPost();
      var hasActiveMetaBoxes = select('core/edit-post').hasMetaBoxes(); // Save metaboxes on save completion when past save wasn't an autosave.
  ...
  INIT: function INIT(_, store) {
    ...
    subscribe(onChangeListener(function () {
      return !!select('core/editor').getBlockSelectionStart();
