 ...
 */
import { castArray, flatMap, find, first, get, has, includes, isArray, isBoolean, last, map, orderBy, reduce, size, some } from 'lodash';
import createSelector from 'rememo';
/**
 * WordPress dependencies
 */

import { serialize, getBlockType, getBlockTypes, hasBlockSupport, hasChildBlocksWithInserterSupport, getFreeformContentHandlerName, isUnmodifiedDefaultBlock } from '@wordpress/blocks';
...
export var getBlockDependantsCacheBust = createSelector(function () {
  return [];
}, function (state, clientId) {
  return map(getBlockOrder(state, clientId), function (innerBlockClientId) {
...
export var getBlock = createSelector(function (state, clientId) {
  ...
  if (type) {
    attributes = reduce(type.attributes, function (result, value, key) {
  ...
  return [state.editor.present.blocks.byClientId[clientId], getBlockDependantsCacheBust(state, clientId), state.editor.present.edits.meta, state.initialEdits.meta, state.currentPost.meta];
});

function getPostMeta(state, key) {
  return has(state, ['editor', 'present', 'edits', 'meta', key]) ? get(state, ['editor', 'present', 'edits', 'meta', key]) : get(state, ['currentPost', 'meta', key]);
}
/**
...
export var getClientIdsOfDescendants = function getClientIdsOfDescendants(state, clientIds) {
  return flatMap(clientIds, function (clientId) {
    ...
    return _toConsumableArray(descendants).concat(_toConsumableArray(getClientIdsOfDescendants(state, descendants)));
  });
};
/**
...
export var getClientIdsWithDescendants = createSelector(function (state) {
  ...
  return _toConsumableArray(topLevelIds).concat(_toConsumableArray(getClientIdsOfDescendants(state, topLevelIds)));
}, function (state) {
...
export var getGlobalBlockCount = createSelector(function (state, blockName) {
  ...
  return reduce(state.editor.present.blocks.byClientId, function (count, block) {
...
export var getBlockRootClientId = createSelector(function (state, clientId) {
...
export var getBlockHierarchyRootClientId = createSelector(function (state, clientId) {
...
export var getMultiSelectedBlockClientIds = createSelector(function (state) {
...
export var canInsertBlockType = createSelector(function (state, blockName) {
...
export var getInserterItems = createSelector(function (state) {
  ...
  var shouldIncludeReusableBlock = function shouldIncludeReusableBlock(reusableBlock) {
    ...
    var referencedBlockType = getBlockType(referencedBlock.name);
  ...
  var blockTypeInserterItems = getBlockTypes().filter(shouldIncludeBlockType).map(buildBlockTypeInserterItem);
  ...
  return orderBy(_toConsumableArray(blockTypeInserterItems).concat(_toConsumableArray(reusableBlockInserterItems)), ['utility', 'frecency'], ['desc', 'desc']);
}, function (state, rootClientId) {
  return [state.blockListSettings[rootClientId], state.editor.present.blocks, state.preferences.insertUsage, state.settings.allowedBlockTypes, state.settings.templateLock, state.reusableBlocks.data, getBlockTypes()];
});
/**
